...
uniform float _obsx;
uniform float _obsz; // OBSTACLE AVOIDANCE
...
half4 TerrainWaveGrass (inout float4 vertex, float waveAmount, half4 color, float2 v)
{
    ...
    float3 bendDir = normalize(float3(worldPos.x, 0, worldPos.z) - float3(_obsx, 0, _obsz));//direction of obstacle bend
    float distLimit = 1;// how far away does obstacle reach
    float distMulti = (distLimit - min(distLimit, distance(float3(worldPos.x, 0, worldPos.z), float3(_obsx, 0, _obsz)))) / distLimit;
    ...
    vertex.xz += bendDir.xz * distMulti * v.y * v.y * v.x  * 2 ; //ADD OBSTACLE BENDING
