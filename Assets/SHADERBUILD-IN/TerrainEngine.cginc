...
CBUFFER_END
    ...
    uniform float _obsx;
    uniform float _obsy; // OBSTACLE AVOIDANCE
...
fixed4 TerrainWaveGrass (inout float4 vertex, float waveAmount, fixed4 color, appdata_full v)
{
    ...
    float3 bendDir = normalize (float3(worldPos.x,0,worldPos.z) - float3(_obsx,0,_obsy));//direction of obstacle bend
    float distLimit = 3;// how far away does obstacle reach
    float distMulti = (distLimit-min(distLimit,distance(float3(worldPos.x,0,worldPos.z),float3(_obsx,0,_obsy))))/distLimit; //distance falloff
    ...
    vertex.xz += bendDir.xz * distMulti * v.texcoord.y * v.texcoord.y * 2.0; //ADD OBSTACLE BENDING
...
void WavingGrassVert (inout appdata_full v)
{
    ...
    v.color = TerrainWaveGrass (v.vertex, waveAmount, v.color, v);
}
